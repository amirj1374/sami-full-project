...
public interface CustomerEventRepository extends JpaRepository<CustomerEvent, Long> {
    ...
    Page<CustomerEvent> findByTenantIdAndCustomerIdOrderByOccurredAtDesc(
            Long tenantId, Long customerId, Pageable pageable);

    Page<CustomerEvent> findByTenantIdAndCustomerIdAndEventTypeOrderByOccurredAtDesc(
            Long tenantId, Long customerId, String eventType, Pageable pageable);

    /** Re-parents timeline events during a merge (history is never lost). */
    @Modifying
    @Query("UPDATE CustomerEvent e SET e.customerId = :targetId WHERE e.tenantId = :tenantId AND e.customerId = :sourceId")
