...
public final class DeliveryDtos {
    ...
    public record LineResponse(Long id,Long orderLineId,Long productId,Long variantId,String serialNumber,String imei,String name,BigDecimal orderedQuantity,BigDecimal reservedQuantity,BigDecimal previouslyDeliveredQuantity,BigDecimal deliveryQuantity,BigDecimal backorderedQuantity,BigDecimal remainingQuantity){static LineResponse from(DeliveryLine l){Long lineId=l.getOrderLineId()!=null?l.getOrderLineId():(l.getOrderLine()==null?null:l.getOrderLine().getId());return new LineResponse(l.getId(),lineId,l.getProductId(),l.getVariantId(),l.getSerialNumber(),l.getImei(),l.getProductName(),l.getOrderedQuantity(),l.getReservedQuantity(),l.getPreviouslyDeliveredQuantity(),l.getDeliveryQuantity(),l.getBackorderedQuantity(),l.getOrderedQuantity().subtract(l.getPreviouslyDeliveredQuantity()).subtract(l.getDeliveryQuantity()).max(BigDecimal.ZERO));}}
