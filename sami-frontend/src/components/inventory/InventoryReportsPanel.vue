...
<template>
  <section aria-labelledby="inventory-reports-title">
    ...
    <v-card rounded="xl" class="app-data-surface pa-4 mb-4"><v-row dense><v-col cols="6" md="3"><AppPersianDatePicker v-model="filters.from" :label="t('inventory.from')" hide-details /></v-col><v-col cols="6" md="3"><AppPersianDatePicker v-model="filters.to" :label="t('inventory.to')" hide-details /></v-col><v-col cols="12" md="3"><v-btn block color="primary" :loading="loading" @click="load">{{ t('common.apply') }}</v-btn></v-col></v-row></v-card>
