  ...
  <div class="sim-investment-page">
    ...
    <v-window v-model="tab">
      ...
      <v-window-item value="numbers">
        <v-card rounded="xl" class="mb-4"><v-card-text><v-row><v-col cols="12" md="4"><v-text-field v-model="filters.search" :label="t('simInvestment.searchPhone')" prepend-inner-icon="mdi-magnify" clearable @keyup.enter="applyFilters" /></v-col><v-col cols="12" sm="4" md="2"><v-select v-model="filters.numberClass" :items="classes" :item-title="label" :label="t('simInvestment.class')" clearable /></v-col><v-col cols="12" sm="4" md="2"><v-select v-model="filters.confidence" :items="levels" :item-title="label" :label="t('simInvestment.confidence')" clearable /></v-col><v-col cols="12" sm="4" md="2"><v-select v-model="filters.liquidity" :items="levels" :item-title="label" :label="t('simInvestment.liquidity')" clearable /></v-col><v-col cols="12" md="2" class="d-flex align-center"><v-btn block color="primary" @click="applyFilters">{{ t('common.apply') }}</v-btn></v-col></v-row></v-card-text></v-card>
    ...
    <v-dialog v-model="importDialog" :fullscreen="!smAndUp" max-width="700"><v-card><v-card-title>{{ t('simInvestment.importSnapshot') }}</v-card-title><v-card-text><v-file-input v-model="importForm.file" accept=".csv,.xlsx,text/csv,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" :label="t('simInvestment.chooseFile')" prepend-icon="mdi-file-table-outline" :hint="t('simInvestment.fileHint')" persistent-hint /><v-text-field v-model="importForm.sourceCode" class="mt-4" :label="t('simInvestment.sourceCode')" /><AppPersianDatePicker v-model="importForm.observedOn" :label="t('simInvestment.observedOn')" clearable /><v-switch v-model="importForm.fullSnapshot" :label="t('simInvestment.fullSnapshot')" :hint="t('simInvestment.fullSnapshotHint')" persistent-hint /><v-alert type="info" variant="tonal" class="mt-3">{{ t('simInvestment.importSafety') }}</v-alert></v-card-text><v-card-actions><v-spacer /><v-btn @click="importDialog = false">{{ t('common.cancel') }}</v-btn><v-btn color="primary" :disabled="!importForm.file" :loading="importing" @click="runImport">{{ t('common.import') }}</v-btn></v-card-actions></v-card></v-dialog>
