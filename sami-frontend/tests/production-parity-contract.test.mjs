...
test('production topology and build-time defaults are explicit and aligned', () => {
  ...
  const escapedVersion = applicationVersion.replaceAll('.', '\\.')
  ...
  const backendPom = readWorkspace('sami-backend/pom.xml')
  const deploy = readWorkspace('scripts/deploy.ps1')
  const releaseConfig = readWorkspace('scripts/release-config.example.ps1')
  ...
  assert.match(frontendDockerfile, new RegExp(`ARG VITE_APP_VERSION=${escapedVersion}`))
  assert.match(backendDockerfile, new RegExp(`ARG APP_VERSION=${escapedVersion}`))
  assert.match(backendPom, new RegExp(`<version>${escapedVersion}</version>`))
  assert.match(compose, new RegExp(`APP_VERSION:-${escapedVersion}`))
  assert.match(deploy, new RegExp(`\\[string\\]\\$ApplicationVersion = '${escapedVersion}'`))
  assert.match(releaseConfig, new RegExp(`ApplicationVersion\\s+= '${escapedVersion}'`))
  assert.match(deploy, /audit', '--audit-level=high/)
  assert.doesNotMatch(deploy, /--ignore-scripts/)
  assert.match(deploy, /ConvertTo-Json -InputObject \$Body/)
  assert.match(deploy, /BACKEND_IMAGE=\$backendImageReference/)
  assert.match(deploy, /FRONTEND_IMAGE=\$frontendImageReference/)
  assert.match(deploy, /CORS_ALLOWED_ORIGINS=\$baseUrl,http:\/\/localhost:\$frontendPort/)
  assert.match(buildInfo, new RegExp(`VITE_APP_VERSION \\|\\| '${escapedVersion}'`))
})
